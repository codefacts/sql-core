...
public class CRUDService {
                ...
                .map(v -> {
                    if (v instanceof JsonObject) {
                        ...
                        if (_js.containsKey("$date")) {
                            return String.format("'%s'", formatDate(Util.parseDate(_js.getString("$date"))));
                        }
                    } else if (v instanceof Map) {
                        Map<String, String> _js = (Map) v;
                        if (_js.containsKey("$date")) {
                            return String.format("'%s'", formatDate(Util.parseDate(_js.get("$date"))));
                        }
                    }
                    return "?";
    ...
    public void query(Message<JsonObject> message) {
        Promises.from(message.body())
                ...
                .complete(p -> System.out.println("COMPLETE SQL_QUERY: " + p));
    }
